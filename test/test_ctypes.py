...
from pudge import numa

class TestNuma():
    def test_available(self):
        assert True == numa.available()
    ...
    def test_nodemask(self):
        ...
        assert set([0]) == numa.numa_nodemask_to_set(numa.set_to_numa_nodemask(set([0])))

    def test_interleave(self):
        numa.set_interleave_mask(set([0]))
        assert set([0]) == numa.get_interleave_mask()
    ...
    def test_membind(self):
        numa.set_membind([0])
        pytest(set([0]), numa.get_membind())

    def test_run_on_nodemask(self):
        numa.set_run_on_node_mask(set([0]))
        assert set([0]) == numa.get_run_on_node_mask()

    def test_get_distance(self):
        assert 10 == numa.get_distance(0, 0)

    def test_affinity(self):
        numa.set_affinity(0, set([0]))
        assert set([0]) == numa.get_affinity(0)
